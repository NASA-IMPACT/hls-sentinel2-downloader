...
def collect_metrics():
    ...
    metrics['total_granules_ignore'] =  granule.select().where(granule.ignore_file==True).count()
    metrics['total_granules_in_progress'] =  granule.select().where(granule.in_progress==True).count()
    metrics['total_granules_not_uploaded'] =  granule.select().where(granule.uploaded==False).where(granule.ignore_file==False).count()
    metrics['total_granules_uploaded'] =  granule.select().where(granule.uploaded==True).count()
    ...
    if cpu_count() == 8:
        cpu1,cpu2,cpu3,cpu4,cpu5,cpu6,cpu7,cpu8 = cpu_percent(interval=0, percpu=True)
